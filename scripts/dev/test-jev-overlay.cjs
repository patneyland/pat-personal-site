...
(async()=>{
 ...
 try {
  ...
  await page.route('**/*.supabase.co/**',route=>{
    if(route.request().method()==='POST'){submissions.push(route.request().postDataJSON());return route.fulfill({json:[{id:99}]});}
  ...
  assert.equal(submissions[0][0].player,'JEV');
  assert.equal(submissions[0][0].mode,'classic');
  ...
  console.log('PASS: identical cabinet geometry, single existing canvas, normal-speed play during API latency, real keyboard input, pause cancellation, normal JEV/classic score flow (mocked), draggable/collapsible window.');
