 ...
 try {
  async function setup(delay,answer){
   ...
   await page.route('**/api/jev',async r=>{
    ...
    await r.fulfill({json:{direction:'up',steps:5,turns:[{direction:'up',steps:5}],eats:false,escape:null,latencyMs:delay,model:'TEST-FIXTURE',...answer}}).catch(()=>{});
   });
  ...
  assert.equal(boards[0].tick,5);assert.equal(boards[0].snake[0].x,13);
  assert.equal(boards[1].tick,10,'Next plan is requested for future segment endpoint');
  ...
  const late=await setup(900);
  await late.page.waitForTimeout(1050);
